...
class Page(models.Model):
    ...
    slug = models.CharField(max_length=50, unique=True, db_index=True, verbose_name=u'Slug',
                            help_text=u'页面的 URL 名称。可包含字母、数字、减号、下划线，不能是以下词语之一：archives、post、tag')
...
class Post(models.Model):
    '''文章'''
    title = models.CharField(max_length=200, verbose_name=u'标题')
    slug = models.CharField(max_length=50, blank=True, verbose_name=u'Slug',
                            help_text=u'本文的短标签，将出现在文章 URL 中。可包含字母、数字、减号、下划线，如：does-python-optimize-function-calls-from-loops')
    author = models.ForeignKey(User, editable=False)
    markdown = models.TextField(verbose_name=u'内容')
    content = models.TextField(blank=True, editable=False)
    created_at = models.DateTimeField(auto_now_add=True, verbose_name=u'发布日期')
...
class Link(models.Model):
    ...
    url = models.URLField(max_length=50, verbose_name=u'链接')  #TODO
    seq = models.IntegerField(default=0, db_index=True, verbose_name=u'排序')
